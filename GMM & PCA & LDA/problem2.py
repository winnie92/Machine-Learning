...
#     C = np.dot(Xc.T,Xc)/(n-1)
    C = np.matrix(np.cov(Xc,rowvar=0))
    print type(C)
    #########################################
    return C
#--------------------------
...
def compute_P(E,v,k):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    v_index = np.argsort(v)                  #sort v 
    E_index = v_index[-1:-(k+1):-1]        #pick index of k largest
    P =E[:,E_index]                      #pick k largest of E
